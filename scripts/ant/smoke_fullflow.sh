#!/bin/bash
# Smoke test for Ant Full Flow - verify config works before full training
# Runs 5 epochs only to validate setup

#SBATCH --job-name=ant_smoke_fullflow
...
#SBATCH --output=logs/slurm/ant/smoke_fullflow_%j.out
#SBATCH --error=logs/slurm/ant/smoke_fullflow_%j.err
