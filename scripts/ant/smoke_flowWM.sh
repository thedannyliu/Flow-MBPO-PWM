#!/bin/bash
# Smoke test for Ant Flow WM - verify config works before full training
# Runs 5 epochs only to validate setup

#SBATCH --job-name=ant_smoke_flowWM
...
#SBATCH --output=logs/slurm/ant/smoke_flowWM_%j.out
#SBATCH --error=logs/slurm/ant/smoke_flowWM_%j.err
