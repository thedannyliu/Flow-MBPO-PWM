#!/bin/bash
# Submit all Ant training experiments
# WandB project: flow-mbpo-single-task-ant
# Variants: Baseline, Flow WM (K=2,4,8), Flow Policy, Full Flow
...
TIME="40:00:00"
CPUS="16"
WANDB_PROJECT="flow-mbpo-single-task-ant"

echo "==========================================="
echo "Submitting ALL Ant Training Experiments"
echo "WandB Project: ${WANDB_PROJECT}"
echo "Time: $(date)"
echo "==========================================="

mkdir -p $PROJECT_ROOT/logs/slurm/ant

SEEDS=(42 123 456)

# =============================================
# Baseline: MLP WM + MLP Policy (3 seeds)
# =============================================
echo ""
echo "=== Baseline (MLP WM + MLP Policy) ==="
for SEED in "${SEEDS[@]}"; do
    JOB_NAME="ant_baseline_s${SEED}"
    RUN_NAME="Ant_Baseline_MLP_s${SEED}"
...
#SBATCH --output=logs/slurm/ant/${JOB_NAME}_%j.out
#SBATCH --error=logs/slurm/ant/${JOB_NAME}_%j.err
...
for SEED in "${SEEDS[@]}"; do
    JOB_NAME="ant_flowWM_K4_s${SEED}"
    RUN_NAME="Ant_FlowWM_K4Heun_s${SEED}"
...
for SEED in "${SEEDS[@]}"; do
    JOB_NAME="ant_flowWM_K2_s${SEED}"
    RUN_NAME="Ant_FlowWM_K2Heun_s${SEED}"
...
for SEED in "${SEEDS[@]}"; do
    JOB_NAME="ant_flowWM_K8_s${SEED}"
    RUN_NAME="Ant_FlowWM_K8Euler_s${SEED}"
...
for SEED in "${SEEDS[@]}"; do
    JOB_NAME="ant_flowpolicy_s${SEED}"
    RUN_NAME="Ant_FlowPolicy_MLPWM_s${SEED}"
...
for SEED in "${SEEDS[@]}"; do
    JOB_NAME="ant_fullflow_s${SEED}"
    RUN_NAME="Ant_FullFlow_FlowWM_FlowPol_s${SEED}"
...
echo ""
echo "==========================================="
echo "All Ant experiments submitted!"
echo ""
echo "Summary:"
echo "  - Baseline: 3 jobs"
