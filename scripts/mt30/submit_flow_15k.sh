#!/bin/bash
#SBATCH --job-name=mt30_flow_15k
#SBATCH --output=logs/slurm/mt30/flow_15k_%A_%a.out
#SBATCH --error=logs/slurm/mt30/flow_15k_%A_%a.err
...
# MT30 Full Flow 15K: High Precision Flow WM + Flow Policy
...
python -u scripts/train_multitask.py -cn config_mt30 \
  ...
  ++wandb.group=epoch_sweep_15k_flow \
  ++wandb.name=flow_15k_${TASK}_s${SEED} \
  hydra.run.dir="outputs/epoch_sweep/flow_15k/${SLURM_JOB_ID}/${SLURM_ARRAY_TASK_ID}_s${SEED}"
