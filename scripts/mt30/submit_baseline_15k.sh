#!/bin/bash
#SBATCH --job-name=mt30_baseline_15k
#SBATCH --output=logs/slurm/mt30/baseline_15k_%A_%a.out
#SBATCH --error=logs/slurm/mt30/baseline_15k_%A_%a.err
...
# MT30 Baseline 15K: Aligned with Original PWM (baselines/original_pwm)
...
python -u scripts/train_multitask.py -cn config_mt30 \
  ...
  ++wandb.group=epoch_sweep_15k_baseline \
  ++wandb.name=baseline_15k_${TASK}_s${SEED} \
  hydra.run.dir="outputs/epoch_sweep/baseline_15k/${SLURM_JOB_ID}/${SLURM_ARRAY_TASK_ID}_s${SEED}"
