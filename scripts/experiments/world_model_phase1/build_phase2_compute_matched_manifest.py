...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--wandb-project", default="flow-mbpo-phase2-compute-matched-sidecar")
...
def main() -> None:
    ...
    rows = []
    stage = f"phase2_compute_matched_{args.mode}"
    for ds in datasets:
        ...
        for budget in budgets:
            for method_key, method_label, profile, alg_config in profiles:
                budget_profile = f"{profile}_it{budget:04d}"
                for seed in seeds:
                    rows.append(
                        {
                            "phase": "phase2",
