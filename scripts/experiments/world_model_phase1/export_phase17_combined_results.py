...
def main() -> None:
    ...
    for stage in args.stages:
        pattern = f"scripts/outputs/world_model_phase1/{stage}/*/*/*/*/seed_*/phase1_summary.json"
        for path in sorted(glob.glob(pattern)):
            ...
            method_key = parts[-4]
            profile = parts[-3]
            rows[(stage, task_key, dataset_key, method_key, profile)].append(d)
    ...
    with out.open("w", newline="", encoding="utf-8") as f:
        ...
        writer.writeheader()
        for key, ds in sorted(rows.items()):
            stage, task_key, dataset_key, method_key, profile = key
            writer.writerow(
                {
                    "stage": stage,
                    "task_key": task_key,
