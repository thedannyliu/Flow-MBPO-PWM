...
RUN_LABEL="${RUN_LABEL:-lewm_fm_ode_train_${GPU_TYPE}_20260603}"
...
cat >"${MANIFEST}" <<'CSV'
row,task,predictor_arch,seed,max_epochs,limit_train_batches,limit_val_batches,ode_substeps,ode_integrator,fm_weight
0,pusht,mlp,0,1,8,2,4,heun,0.0
1,pusht,fm_ode,0,1,8,2,4,heun,1.0
2,pusht,mlp,1,1,8,2,4,heun,0.0
3,pusht,fm_ode,1,1,8,2,4,heun,1.0
CSV

lewm_fm_ode_train_job="$(
  sbatch --parsable \
    --job-name="lewm_fm_ode_${GPU_TYPE}" \
    --account="${ACCOUNT}" \
...
model_name="${run_name}"
run_out="${OUT_DIR}/${task}/predictor_${predictor_arch}/seed_${seed}"
mkdir -p "${run_out}"
cat >"${run_out}/run.json" <<EOF
{"row": ${row}, "task": "${task}", "predictor_arch": "${predictor_arch}", "seed": ${seed}, "max_epochs": ${max_epochs}, "limit_train_batches": ${limit_train_batches}, "limit_val_batches": ${limit_val_batches}, "ode_substeps": ${ode_substeps}, "ode_integrator": "${ode_integrator}", "fm_weight": ${fm_weight}, "output_model_name": "${model_name}", "slurm_job_id": "${SLURM_ARRAY_JOB_ID}", "slurm_array_task_id": "${SLURM_ARRAY_TASK_ID}"}
EOF
cd "${LEWM_ROOT}"
export PYTHONNOUSERSITE=1
export PYTHONPATH="${PATCH_ROOT}:${COMPAT_ROOT}:${LEWM_ROOT}:${PYTHONPATH:-}"
export STABLEWM_HOME="${STABLEWM_HOME}"
export LEWM_CONFIG_DIR="${LEWM_ROOT}/config/train"
export WANDB_MODE=disabled
export MUJOCO_GL=egl
export HYDRA_FULL_ERROR=1
"${LEWM_ENV}/bin/python" "${PATCH_ROOT}/run_lewm_fm_ode_train.py" \
  data=pusht \
...
cat <<EOF
lewm_fm_ode_train_job=${lewm_fm_ode_train_job}
manifest=${MANIFEST}
output_dir=${OUT_DIR}
EOF
