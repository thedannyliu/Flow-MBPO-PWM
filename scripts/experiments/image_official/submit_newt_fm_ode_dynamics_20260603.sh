...
RUN_LABEL="${RUN_LABEL:-newt_fm_ode_dynamics_${GPU_TYPE}_20260603}"
...
SITE_ROOT="${PATCH_ROOT}/newt_fm_ode_site"
...
cat >"${MANIFEST}" <<'CSV'
row,task,dynamics_arch,seed,steps,model_size,ode_substeps,ode_integrator,fm_weight,endpoint_weight
0,walker-run,mlp,0,5000,B,4,heun,0.0,1.0
1,walker-run,fm_ode,0,5000,B,4,heun,1.0,1.0
2,walker-run,mlp,1,5000,B,4,heun,0.0,1.0
3,walker-run,fm_ode,1,5000,B,4,heun,1.0,1.0
CSV

newt_fm_ode_job="$(
  sbatch --parsable \
    --job-name="newt_fm_ode_${GPU_TYPE}" \
    --account="${ACCOUNT}" \
...
IFS=, read -r _ task dynamics_arch seed steps model_size ode_substeps ode_integrator fm_weight endpoint_weight <<<"${line}"
export NEWT_FM_ODE_DYNAMICS=0
[[ "${dynamics_arch}" == "fm_ode" ]] && export NEWT_FM_ODE_DYNAMICS=1
export NEWT_FM_ODE_SUBSTEPS="${ode_substeps}"
export NEWT_FM_ODE_INTEGRATOR="${ode_integrator}"
export NEWT_FM_LOSS_WEIGHT="${fm_weight}"
export NEWT_FM_ENDPOINT_WEIGHT="${endpoint_weight}"
run_name="${RUN_LABEL}_${task}_dyn${dynamics_arch}_seed${seed}"
run_out="${OUT_DIR}/${task}/dynamics_${dynamics_arch}/seed_${seed}"
mkdir -p "${run_out}"
cat >"${run_out}/run.json" <<EOF
{"row": ${row}, "task": "${task}", "dynamics_arch": "${dynamics_arch}", "seed": ${seed}, "steps": ${steps}, "model_size": "${model_size}", "ode_substeps": ${ode_substeps}, "ode_integrator": "${ode_integrator}", "fm_weight": ${fm_weight}, "endpoint_weight": ${endpoint_weight}, "slurm_job_id": "${SLURM_ARRAY_JOB_ID}", "slurm_array_task_id": "${SLURM_ARRAY_TASK_ID}"}
EOF
cd "${NEWT_ROOT}/tdmpc2"
...
cat <<EOF
newt_fm_ode_job=${newt_fm_ode_job}
manifest=${MANIFEST}
output_dir=${OUT_DIR}
EOF
