...
def prepare_replay(buffer: dict[str, torch.Tensor], lambda_uncertainty: float, termination_quantile: float) -> dict[str, torch.Tensor]:
    ...
    replay["done"] = done | uncertainty_done
...
def main() -> None:
    ...
    replay = prepare_replay(buffer, args.lambda_uncertainty, args.uncertainty_quantile_termination)
