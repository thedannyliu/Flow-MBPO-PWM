#!/bin/bash
# Smoke test for Ant Flow Policy - verify config works before full training
# Runs 5 epochs only to validate setup

#SBATCH --job-name=ant_smoke_flowpolicy
...
#SBATCH --output=logs/slurm/ant/smoke_flowpolicy_%j.out
#SBATCH --error=logs/slurm/ant/smoke_flowpolicy_%j.err
