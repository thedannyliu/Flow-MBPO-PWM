...
class ResidualFlowWorldModel(nn.Module):
    """MLP transition backbone plus a flow-style residual correction."""
    ...
    def __init__(
        ...
        task_dim=0,
        residual_substeps=4,
        residual_integrator="heun",
        freeze_base_after_pretrain=False,
        base_pretrain_iters=0,
        ...
        self.freeze_base_after_pretrain = bool(freeze_base_after_pretrain)
        self.base_pretrain_iters = int(base_pretrain_iters)
        ...
        # Residual velocity sees context z_t, current residual state, action, and tau.
        self._velocity = mlp(
            latent_dim * 2 + action_dim + 1 + task_dim,
            units,
            latent_dim,
            last_layer=dynamics["last_layer"],
            last_layer_kwargs=dynamics["last_layer_kwargs"],
    ...
    def freeze_base(self):
        for module in (self._encoder, self._dynamics):
    ...
    def residual_velocity(self, z_context, r, a, tau, task):
        x = torch.cat([z_context, r, a, tau], dim=-1)
        if self.multitask:
            x = self.task_emb(x, task)
        else:
    ...
    def residual_vector(self, z, a, task, substeps=None, integrator=None):
        substeps = int(substeps or self.residual_substeps)
        integrator = integrator or self.residual_integrator
        dt = 1.0 / max(1, substeps)
        r = torch.zeros_like(z)
        for k in range(max(1, substeps)):
            tau = torch.full((z.shape[0], 1), k * dt, device=z.device, dtype=z.dtype)
            k1 = self.residual_velocity(z, r, a, tau, task)
            if integrator == "euler":
                r = r + dt * k1
                ...
                k2 = self.residual_velocity(z, r_pred, a, tau_next, task)
                r = r + 0.5 * dt * (k1 + k2)
            else:
                raise ValueError(f"Unknown residual integrator: {integrator}")
        return r

    def next(self, z, a, task, integrator=None, substeps=None, **_):
        base = self.base_next(z, a, task)
        residual = self.residual_vector(z, a, task, substeps=substeps, integrator=integrator)
