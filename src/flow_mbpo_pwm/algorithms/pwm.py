...
class PWM:
    ...
    def __init__(
        ...
        self.use_flow_dynamics = use_flow_dynamics
        ...
        self.log = log
        self.log_dir = Path(logdir)  # Convert to Path object for proper path operations
        self.log_dir.mkdir(parents=True, exist_ok=True)
        
        # Initialize enhanced monitoring tools
        ...
        # Initialize WandB logger (will be activated in train() if enabled)
        ...
        # Log world model parameter count
        wm_params = self.wm.total_params
        print_info(f"World Model Total Parameters: {wm_params:,}")
        if self.use_flow_dynamics:
            print_info(f"Using Flow-Matching Dynamics:")
    ...
    def eval(self, num_games, deterministic=True):
        ...
        while games_cnt < num_games:
            ...
            actions = torch.tanh(actions)
            # world model step may return either (z, rew, trunc) or (z, rew)
            # (different world model implementations). Handle both to avoid
            ...
            try:
                ...
                # Fallback: if world model returned (z, rew), create a
                # zeroed truncation mask (no truncations). This preserves
                # backward compatibility with older/newer world model APIs.
                z, rew = res
                trunc = None  # Will be created after reward conversion

            # Convert reward distribution to scalar if needed (MUST happen before using rew.shape)
            if rew.ndim > 1 and rew.shape[-1] > 1:
                rew = self.wm.two_hot_inv(rew)
            # Ensure rew is 1D tensor [batch_size] not [batch_size, 1]
            if rew.ndim > 1:
                rew = rew.squeeze(-1)
                
            # Create trunc with correct shape if it wasn't provided
            if trunc is None:
                trunc = torch.zeros(
                    rew.shape[0], dtype=torch.bool, device=rew.device
                )
    ...
    def train(self):
        ...
        if self.visualizer:
            try:
                ...
                print(f"Generated training visualizations in {self.log_dir}")
            except Exception as e:
    ...
    def load(self, path, buffer=False, resume_training=False):
        ...
        Args:
            ...
            buffer: Whether to also load replay buffer
        ...
        # Load optimizer states and learning rates
        ...
        # Restore training progress if resuming
