...
| `9400409_[4-15%8]` | Remaining official NEWT broad A100 smoke rows | `PENDING`, reason `Priority` | Same official NEWT broad smoke payload | Existing submission record | Official NEWT broad smoke rows 4-15 | Same official NEWT repo/env/data | seeds/tasks by row | A100 / `embers` | W&B disabled | No checkpoint expected | Logs pending | Not available | None yet | Pending only | Wait; L40S/H200 coverage already provides infrastructure signal, so no duplicate submission is needed |
