...
| `9400409_5` | Official NEWT broad A100 smoke, pendulum-swingup seed0 | `RUNNING` at poll | Same official NEWT broad smoke payload | Existing submission record | Official NEWT `tdmpc2/train.py`, task `pendulum-swingup`, seed 0, `steps=500`, `model_size=B`, W&B disabled | Same official NEWT repo/env/data | seed `0` | A100 / `embers` | W&B disabled | No checkpoint expected | Log started at `logs/slurm/image_official/newt_official_broad_smoke_9400409_5.out` | Not available yet | None yet | Running only | Inspect after completion before recording rewards |
| `9400409_[6-15%8]` | Remaining official NEWT broad A100 smoke rows | `PENDING`, reason `Priority` | Same official NEWT broad smoke payload | Existing submission record | Official NEWT broad smoke rows 6-15 | Same official NEWT repo/env/data | seeds/tasks by row | A100 / `embers` | W&B disabled | No checkpoint expected | Logs pending | Not available | None yet | Pending only | Wait; L40S/H200 coverage already provides infrastructure signal, so no duplicate submission is needed |
