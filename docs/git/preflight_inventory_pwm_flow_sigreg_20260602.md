...
## Continuation Inventory: Full Upstream PWM W&B Threadfix

Preflight time: 2026-06-03 after commit `7c959cb`, America/New_York.

| Field | Value |
| --- | --- |
| Branch | `mjlab-qs-rollout-policy-improvement` |
| Current HEAD | `7c959cb` |
| Dirty status before this inventory edit | clean after committing the W&B threadfix and AGENTS.md policy update |
| Slurm commands | `squeue -u $USER -o ...`; `sacct -j 9402769,9402771,9402772,9402773,9402774,9402770,... --format=... -P` |
| Artifact searches | Searched `scripts/outputs/mjlab_qs` for `*upstream_pwm_full_pipeline*wandb*summary.json` and rollout MP4 files; H200 rollout MP4 files existed from failed W&B-upload jobs, but no W&B-backed formal summary was accepted. |

| Job ID | Purpose | Status | Command / script | Git SHA | Config | Env / dataset / version | Seed | GPU / QOS | W&B link or offline dir | Checkpoint paths | Eval / video paths | Return / length / fall | Failure reason | Usable? | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `9402769_[0-1]` | Full upstream PWM formal eval40, W&B-on, H200 | `FAILED`, exit `1:0`, each about one minute | `scripts/experiments/mjlab_qs/submit_upstream_pwm_mjlab_formal_eval_rollout_20260603.sh`, `MODE=eval`, H200 | Submitted before `7c959cb`; wrapper recorded earlier SHA | Existing longdiag run `baselines/PWM/scripts/outputs/2026-06-02/21-35-04`; final/best policies | Locked original PWM env plus MJLab project site through `locked_mjlab_python.py`; full upstream `train_dflex.py` checkpoint config | `0` | H200 / `embers` | Intended W&B project `flow-mbpo-mjlab-full-upstream-pwm`; no accepted run URL recorded | `.../upstream_pwm_mjlab_full_longdiag_h200_seed0_20260602/{final_policy.pt,best_policy.pt}` | No accepted formal summary | Not accepted | Old W&B 0.12.21 backend subprocess failed with `ModuleNotFoundError: wandb.sdk.internal.internal` in the mixed locked-PWM/MJLab wrapper | No, infrastructure failure after eval path | Superseded by `9402882` |
| `9402771_[0-1]` | Full upstream PWM formal rollout10, W&B-on, H200 | `FAILED`, exit `1:0`; MP4 files written locally before W&B failure | Same submit wrapper, `MODE=rollout`, H200 | Submitted before `7c959cb` | Same longdiag checkpoint source | Same locked-PWM/MJLab wrapper | `0` | H200 / `embers` | Intended W&B project; no accepted run URL recorded | Same final/best policies | Local MP4 files under `scripts/outputs/mjlab_qs/upstream_pwm_full_pipeline_rollout10_wandb_h200_20260603/{final,best}/rollout.mp4` | Local summaries not accepted as formal W&B evidence | Same W&B backend subprocess failure at upload/logging stage | No for formal claims; useful only as debug evidence that rendering reached MP4 export | Superseded by `9402885` |
| `9402772_0`; `9402772_1`; `9402773`; `9402774`; `9402770` | Full upstream PWM W&B-on peer/backup formal arrays | `9402772_0` failed with same W&B root cause; `9402772_1`, `9402773`, `9402774`, and `9402770` canceled before repeat | Same submit wrapper across H100/L40S eval/rollout | Submitted before `7c959cb` | Same longdiag checkpoint source | Same locked-PWM/MJLab wrapper | `0` | H100/L40S / `embers` | Intended W&B project; no accepted formal runs | Same final/best policies | None accepted | Not accepted | Shared W&B backend root cause; pending jobs canceled to avoid repeating known failure | No | Superseded by `9402883`, `9402884`, `9402887`, and `9402888` |
| `9402882_[0-1%2]` | Full upstream PWM formal eval40 threadfix, H200 | `PENDING`, reason `Resources` at first check | Same submit wrapper after `7c959cb`; `MODE=eval`, H200, output root suffix `wandb_threadfix_h200_20260603` | `7c959cb` | Same longdiag checkpoint source | Same locked-PWM/MJLab wrapper; W&B forced to thread backend | `0` | H200 / `embers` | W&B project `flow-mbpo-mjlab-full-upstream-pwm`, group `upstream_pwm_mjlab_full_pipeline_20260603_eval` | Same final/best policies | Expected under `scripts/outputs/mjlab_qs/upstream_pwm_full_pipeline_formal_eval40_wandb_threadfix_h200_20260603/{final,best}/` | Pending | None yet | Pending formal replacement | Inspect logs and summaries once started |
| `9402885_[0-1%2]` | Full upstream PWM formal rollout10 threadfix, H200 | `PENDING`, reason `Resources` at first check | Same submit wrapper after `7c959cb`; `MODE=rollout`, H200, output root suffix `wandb_threadfix_h200_20260603` | `7c959cb` | Same longdiag checkpoint source | Same locked-PWM/MJLab wrapper; W&B forced to thread backend | `0` | H200 / `embers` | W&B project `flow-mbpo-mjlab-full-upstream-pwm`, group `upstream_pwm_mjlab_full_pipeline_20260603_rollout` | Same final/best policies | Expected under `scripts/outputs/mjlab_qs/upstream_pwm_full_pipeline_rollout10_wandb_threadfix_h200_20260603/{final,best}/` | Pending | None yet | Pending formal replacement | Inspect logs, MP4s, W&B URLs once started |
| `9402884_[0-1%2]`; `9402883_[0-1%2]`; `9402887_[0-1%2]`; `9402888_[0-1%2]` | Full upstream PWM formal threadfix backup arrays on H100/L40S | `PENDING`, H100 reason `Priority`; L40S submitted with `CPUS_PER_TASK=4` | Same submit wrapper after `7c959cb`; distinct output roots for H100/L40S eval/rollout | `7c959cb` | Same longdiag checkpoint source | Same locked-PWM/MJLab wrapper; W&B forced to thread backend | `0` | H100/L40S / `embers` | Same W&B project/group convention | Same final/best policies | Expected under `scripts/outputs/mjlab_qs/upstream_pwm_full_pipeline_{formal_eval40,rollout10}_wandb_threadfix_{h100,l40s}_20260603/{final,best}/` | Pending | None yet | Pending formal backups | Keep pending unless H200 produces complete accepted formal eval and rollout artifacts first |
...
## Continuation Inventory: AWAC H200 Result And Backup Submissions

Preflight/result time: 2026-06-02 after commit `9210e59`, America/New_York.

| Job ID | Purpose | Status | Command / script | Git SHA | Config | Env / dataset / version | Seed | GPU / QOS | W&B link or offline dir | Checkpoint paths | Eval / video paths | Return / length / fall | Failure reason | Usable? | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `9402229_0` | H200 AWAC endpoint H1 diagnostic | `COMPLETED`, exit `0:0`, elapsed `00:00:49` | `submit_array.sh --kind flow_mbpo_awr --manifest flow_mbpo_awac_short_horizon_diag_20260602.csv --gpu-type H200 --partition gpu-h200 --qos embers --max-concurrent 3 --time 02:00:00 --mem 128G --cpus 8 --conda-env pwm` | Summary recorded `52e56dbc7a3864b2ac80e4e8586888785bcc688d` | `advantage_source=critic_awac`, endpoint H1 replay, conservative critic, mixed OOD actions, support action penalty, real eval every 20 | Project `pwm` env; `PYTHONPATH` includes project `src` and `baselines/PWM/src`; MJLab QS H16 dataset; BC seed0 policy; endpoint H1 replay | 0 | H200 / `embers` | W&B disabled | `scripts/outputs/mjlab_qs/flow_mbpo_awac_short_horizon_diag_20260602/endpoint_h1_awac_cql_mixed_evalstop_s0/{final_policy_extraction.pt,best_policy_extraction.pt,best_training_loss_policy_extraction.pt,final_q_critic.pt}` | `summary.json`; `real_eval_snapshots/iter_000020_policy_extraction.pt`; no video for this W&B-disabled diagnostic | return `19.7945`, length `296.875`, fall `1.000`, selection `-77.2368`, BC gate false | None; configured early stop at iter 20 because selection score below -50 | Usable as GPU-node environment success and negative diagnostic, not formal fixed-SHA evidence | Do not expand this exact H1 setting unless backup partitions disagree materially |
| `9402229_1` | H200 AWAC endpoint H3 diagnostic | `COMPLETED`, exit `0:0`, elapsed `00:00:41` | Same H200 array | Summary recorded `52e56dbc7a3864b2ac80e4e8586888785bcc688d` | Same AWAC config with endpoint H3 replay | Same env and dataset; endpoint H3 replay | 0 | H200 / `embers` | W&B disabled | `scripts/outputs/mjlab_qs/flow_mbpo_awac_short_horizon_diag_20260602/endpoint_h3_awac_cql_mixed_evalstop_s0/{final_policy_extraction.pt,best_policy_extraction.pt,best_training_loss_policy_extraction.pt,final_q_critic.pt}` | `summary.json`; `real_eval_snapshots/iter_000020_policy_extraction.pt`; no video | return `19.8573`, length `297.125`, fall `1.000`, selection `-77.1714`, BC gate false | None; configured early stop at iter 20 | Usable as GPU-node environment success and negative diagnostic, not formal fixed-SHA evidence | Do not expand this exact H3 setting unless backup partitions disagree materially |
| `9402229_2` | H200 AWAC endpoint H5 diagnostic | `COMPLETED`, exit `0:0`, elapsed `00:00:43` | Same H200 array | Summary recorded `9210e59e65df8247381739d513c52f396494fcd6` | Same AWAC config with endpoint H5 replay | Same env and dataset; endpoint H5 replay | 0 | H200 / `embers` | W&B disabled | `scripts/outputs/mjlab_qs/flow_mbpo_awac_short_horizon_diag_20260602/endpoint_h5_awac_cql_mixed_evalstop_s0/{final_policy_extraction.pt,best_policy_extraction.pt,best_training_loss_policy_extraction.pt,final_q_critic.pt}` | `summary.json`; `real_eval_snapshots/iter_000020_policy_extraction.pt`; no video | return `19.8948`, length `298.250`, fall `1.000`, selection `-77.1227`, BC gate false | None; configured early stop at iter 20 | Usable as GPU-node environment success and negative diagnostic, not formal fixed-SHA evidence | Do not expand this exact H5 setting unless backup partitions disagree materially |
| `9402277_[0-2%3]` | H100 broad backup for same AWAC H1/H3/H5 diagnostic with distinct output root | `PENDING`, reason `Priority` at first poll | `submit_array.sh --kind flow_mbpo_awr --manifest flow_mbpo_awac_short_horizon_diag_h100_20260602.csv --gpu-type H100 --partition gpu-h100 --qos embers --max-concurrent 3 --time 02:00:00 --mem 128G --cpus 8 --conda-env pwm` | `9210e59` at submission | Same AWAC rows, H100 output root | Same env/dataset/replays | 0 | H100 / `embers` | W&B disabled | Pending | Pending | Pending | None yet | Pending backup | Monitor; keep queued per GPU-fill instruction |
| `9402279_[0-2%3]` | A100 broad backup for same AWAC H1/H3/H5 diagnostic with distinct output root | `PENDING`, reason `Priority` at first poll | Same as above, manifest `flow_mbpo_awac_short_horizon_diag_a100_20260602.csv`, gpu-a100, 8 CPUs | `9210e59` at submission | Same AWAC rows, A100 output root | Same env/dataset/replays | 0 | A100 / `embers` | W&B disabled | Pending | Pending | Pending | None yet | Pending backup | Monitor; keep queued per GPU-fill instruction |
| `9402278_[0-2%3]` | L40S broad backup for same AWAC H1/H3/H5 diagnostic with distinct output root | `PENDING`, reason `Priority` at first poll | Same as above, manifest `flow_mbpo_awac_short_horizon_diag_l40s_20260602.csv`, gpu-l40s, 4 CPUs | `9210e59` at submission | Same AWAC rows, L40S output root | Same env/dataset/replays | 0 | L40S / `embers` | W&B disabled | Pending | Pending | Pending | None yet | Pending backup; 4 CPUs used because L40S rejects 8 CPUs with CPU:GPU ratio 4:1 | Monitor; keep queued per GPU-fill instruction |

Interpretation: the AWAC test environment is now confirmed on a GPU node:
H200 rows initialized MJLab, ran the `critic_awac` update path, wrote policy and
critic checkpoints, ran real-env eval, and exited 0:0. The performance result is
negative: all three H200 rows remain below the BC gate and fall at rate 1.000.
...
## Continuation Inventory: AWAC Completed H200/H100/L40S Results

Result time: 2026-06-02 after commit `b684ea4`, America/New_York.

| Job ID | Purpose | Status | Command / script | Git SHA | Config | Env / dataset / version | Seed | GPU / QOS | W&B link or offline dir | Checkpoint paths | Eval / video paths | Return / length / fall | Failure reason | Usable? | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `9402337_0` | Fixed-SHA seed1 endpoint H1 AWAC | `COMPLETED`, exit `0:0`, elapsed `00:00:29` | `submit_array.sh --kind flow_mbpo_awr --manifest flow_mbpo_awac_fixedsha_seed1_h200_20260602.csv --gpu-type H200 --partition gpu-h200 --qos embers --max-concurrent 3 --time 02:00:00 --mem 128G --cpus 8 --conda-env pwm` | `e1ec4df49d0020b2e2ab42a30376b1dc0264d72e` | `advantage_source=critic_awac`, endpoint H1, conservative critic, mixed OOD, real eval every 20 | Project `pwm` env; MJLab QS H16 dataset; BC seed0 policy; endpoint H1 replay | 1 | H200 / `embers` | W&B disabled | Row dir under `scripts/outputs/mjlab_qs/flow_mbpo_awac_fixedsha_seed1_h200_20260602/endpoint_h1_awac_cql_mixed_evalstop_s1/` includes final/best policies and final critic | `summary.json`; real-eval snapshot; no video | `8.9520 / 159.250 / 1.000`, selection `-89.4555`, BC gate false | None; configured early stop at iter 20 | Usable fixed-SHA negative diagnostic | Do not promote this setting |
| `9402337_1` | Fixed-SHA seed1 endpoint H3 AWAC | `COMPLETED`, exit `0:0`, elapsed `00:00:27` | Same H200 array | `e1ec4df49d0020b2e2ab42a30376b1dc0264d72e` | Endpoint H3 AWAC | Same env/dataset; endpoint H3 replay | 1 | H200 / `embers` | W&B disabled | Row dir under fixed-SHA H200 output root | `summary.json`; real-eval snapshot; no video | `8.5052 / 154.375 / 1.000`, selection `-89.9511`, BC gate false | None; configured early stop at iter 20 | Usable fixed-SHA negative diagnostic | Do not promote this setting |
| `9402337_2` | Fixed-SHA seed1 endpoint H5 AWAC | `COMPLETED`, exit `0:0`, elapsed `00:00:28` | Same H200 array | `e1ec4df49d0020b2e2ab42a30376b1dc0264d72e` | Endpoint H5 AWAC | Same env/dataset; endpoint H5 replay | 1 | H200 / `embers` | W&B disabled | Row dir under fixed-SHA H200 output root | `summary.json`; real-eval snapshot; no video | `8.8065 / 158.000 / 1.000`, selection `-89.6135`, BC gate false | None; configured early stop at iter 20 | Usable fixed-SHA negative diagnostic | Do not promote this setting |
| `9402361_0` | Replay-family trajectory H3 AWAC | `COMPLETED`, exit `0:0`, elapsed `00:00:30` | `submit_array.sh --kind flow_mbpo_awr --manifest flow_mbpo_awac_replay_family_seed0_h200_20260602.csv --gpu-type H200 --partition gpu-h200 --qos embers --max-concurrent 5 --time 02:00:00 --mem 128G --cpus 8 --conda-env pwm` | `9b5319d24f92ad0a4ebd40814905f771affc8628` | `advantage_source=critic_awac`, trajectory chunk H3 replay | Same env/dataset; trajectory H3 replay | 0 | H200 / `embers` | W&B disabled | Row dir under `scripts/outputs/mjlab_qs/flow_mbpo_awac_replay_family_seed0_h200_20260602/traj_h3_ensemble_awac_cql_mixed_evalstop_s0/` | `summary.json`; real-eval snapshot; no video | `19.4614 / 292.000 / 1.000`, selection `-77.6186`, BC gate false | None; configured early stop at iter 20 | Usable fixed-SHA negative diagnostic | Do not expand this exact setting |
| `9402361_1` | Replay-family trajectory H5 AWAC | `COMPLETED`, exit `0:0`, elapsed `00:00:30` | Same H200 replay-family array | `9b5319d24f92ad0a4ebd40814905f771affc8628` | Trajectory chunk H5 replay | Same env/dataset; trajectory H5 replay | 0 | H200 / `embers` | W&B disabled | Row dir under replay-family H200 output root | `summary.json`; real-eval snapshot; no video | `19.4218 / 291.750 / 1.000`, selection `-77.6607`, BC gate false | None; configured early stop at iter 20 | Usable fixed-SHA negative diagnostic | Do not expand this exact setting |
| `9402361_2` | Replay-family residual H3 AWAC | `COMPLETED`, exit `0:0`, elapsed `00:00:32` | Same H200 replay-family array | `9b5319d24f92ad0a4ebd40814905f771affc8628` | Residual-flow H3 replay | Same env/dataset; residual H3 replay | 0 | H200 / `embers` | W&B disabled | Row dir under replay-family H200 output root | `summary.json`; real-eval snapshot; no video | `19.9263 / 298.125 / 1.000`, selection `-77.0925`, BC gate false | None; configured early stop at iter 20 | Usable fixed-SHA negative diagnostic | Do not expand this exact setting |
| `9402361_3` | Replay-family residual H5 AWAC | `COMPLETED`, exit `0:0`, elapsed `00:00:28` | Same H200 replay-family array | `9b5319d24f92ad0a4ebd40814905f771affc8628` | Residual-flow H5 replay | Same env/dataset; residual H5 replay | 0 | H200 / `embers` | W&B disabled | Row dir under replay-family H200 output root | `summary.json`; real-eval snapshot; no video | `19.5381 / 292.500 / 1.000`, selection `-77.5369`, BC gate false | None; configured early stop at iter 20 | Usable fixed-SHA negative diagnostic | Do not expand this exact setting |
| `9402361_4` | Replay-family MLP-ref H1 AWAC | `COMPLETED`, exit `0:0`, elapsed `00:00:30` | Same H200 replay-family array | `9b5319d24f92ad0a4ebd40814905f771affc8628` | MLP-reference H1 replay | Same env/dataset; MLP-ref H1 replay | 0 | H200 / `embers` | W&B disabled | Row dir under replay-family H200 output root | `summary.json`; real-eval snapshot; no video | `19.5179 / 293.000 / 1.000`, selection `-77.5521`, BC gate false | None; configured early stop at iter 20 | Usable fixed-SHA negative diagnostic | Do not expand this exact setting |
| `9402277_[0-2%3]` | Old H100 endpoint AWAC backup submitted before metadata fix | all three rows `COMPLETED`, exit `0:0` | `flow_mbpo_awac_short_horizon_diag_h100_20260602.csv`, H100 / embers | Summaries recorded live SHA `b684ea4e2cc699da7d7202baeebca93ddfb19980` because wrapper was submitted before metadata fix | Endpoint H1/H3/H5 AWAC | Same env/dataset/replays | 0 | H100 / `embers` | W&B disabled | Per-row output dirs under `flow_mbpo_awac_short_horizon_diag_h100_20260602/` | `summary.json`; real-eval snapshots; no video | H1 `19.7080 / 295.000 / 1.000`; H3 `19.6832 / 295.250 / 1.000`; H5 `19.9007 / 297.500 / 1.000`; all BC gate false | None; configured early stop at iter 20 | Usable negative diagnostic, not fixed-SHA evidence | No promotion |
| `9402278_[0-2%3]` | Old L40S endpoint AWAC backup submitted before metadata fix | all three rows `COMPLETED`, exit `0:0` | `flow_mbpo_awac_short_horizon_diag_l40s_20260602.csv`, L40S / embers, 4 CPUs | Summaries recorded live SHA `b684ea4e2cc699da7d7202baeebca93ddfb19980` because wrapper was submitted before metadata fix | Endpoint H1/H3/H5 AWAC | Same env/dataset/replays | 0 | L40S / `embers` | W&B disabled | Per-row output dirs under `flow_mbpo_awac_short_horizon_diag_l40s_20260602/` | `summary.json`; real-eval snapshots; no video | H1 `19.9827 / 298.375 / 1.000`; H3 `19.0628 / 277.125 / 1.000`; H5 `19.5757 / 293.500 / 1.000`; all BC gate false | None; configured early stop at iter 20 | Usable negative diagnostic, not fixed-SHA evidence | No promotion |

Interpretation: the completed AWAC diagnostics now cover endpoint seed0/seed1,
trajectory chunk, residual-flow, and MLP-reference replay families across H200
plus H100/L40S endpoint backups. Every completed row is below the BC gate
`45.8491 / 594.97 / 0.625`, falls at rate `1.000`, and early-stops at iter 20.
This closes the current AWAC setting as negative; remaining A100/H100/L40S
pending rows can confirm portability but are unlikely to change the mechanism
conclusion unless they materially differ.

## Continuation Inventory: AWAC H100/L40S Fixed-SHA And Replay-Family Results

Result time: 2026-06-02 after commit `dcdfe10`, America/New_York.

| Job ID | Purpose | Status | Command / script | Git SHA | Config | Env / dataset / version | Seed | GPU / QOS | W&B link or offline dir | Checkpoint paths | Eval / video paths | Return / length / fall | Failure reason | Usable? | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `9402338_[0-2%3]` | H100 fixed-SHA endpoint seed1 AWAC H1/H3/H5 | all three rows `COMPLETED`, exit `0:0` | `flow_mbpo_awac_fixedsha_seed1_h100_20260602.csv`, H100 / embers, 8 CPUs | `e1ec4df49d0020b2e2ab42a30376b1dc0264d72e` | Endpoint H1/H3/H5, `advantage_source=critic_awac`, conservative critic, mixed OOD, real eval every 20 | Project `pwm` env; MJLab QS H16 dataset; BC seed0 policy; endpoint replay files | 1 | H100 / `embers` | W&B disabled | Per-row output dirs under `flow_mbpo_awac_fixedsha_seed1_h100_20260602/` | `summary.json`; real-eval snapshots; no video | H1 `8.9832 / 159.250 / 1.000`; H3 `8.5861 / 154.000 / 1.000`; H5 `8.8278 / 157.625 / 1.000`; all BC gate false | None; configured early stop at iter 20 | Usable fixed-SHA negative diagnostic | No promotion |
| `9402339_[0-2%3]` | L40S fixed-SHA endpoint seed1 AWAC H1/H3/H5 | all three rows `COMPLETED`, exit `0:0` | `flow_mbpo_awac_fixedsha_seed1_l40s_20260602.csv`, L40S / embers, 4 CPUs | `e1ec4df49d0020b2e2ab42a30376b1dc0264d72e` | Same endpoint AWAC config | Same env/dataset/replays | 1 | L40S / `embers` | W&B disabled | Per-row output dirs under `flow_mbpo_awac_fixedsha_seed1_l40s_20260602/` | `summary.json`; real-eval snapshots; no video | H1 `11.3574 / 192.375 / 1.000`; H3 `9.3057 / 165.125 / 1.000`; H5 `9.1035 / 161.375 / 1.000`; all BC gate false | None; configured early stop at iter 20 | Usable fixed-SHA negative diagnostic | No promotion |
| `9402364_[0-4%5]` | H100 replay-family seed0 AWAC | all five rows `COMPLETED`, exit `0:0` | `flow_mbpo_awac_replay_family_seed0_h100_20260602.csv`, H100 / embers, 8 CPUs | `9b5319d24f92ad0a4ebd40814905f771affc8628` | Trajectory H3/H5, residual H3/H5, MLP-ref H1, `advantage_source=critic_awac` | Same env/dataset/replays | 0 | H100 / `embers` | W&B disabled | Per-row output dirs under `flow_mbpo_awac_replay_family_seed0_h100_20260602/` | `summary.json`; real-eval snapshots; no video | traj_h3 `19.1829 / 288.000 / 1.000`; traj_h5 `18.6628 / 272.000 / 1.000`; residual_h3 `19.4707 / 291.500 / 1.000`; residual_h5 `19.3436 / 289.875 / 1.000`; mlp_ref_h1 `18.7725 / 273.375 / 1.000`; all BC gate false | None; configured early stop at iter 20 | Usable fixed-SHA negative diagnostic | No promotion |
| `9402363_[0-4%5]` | L40S replay-family seed0 AWAC | all five rows `COMPLETED`, exit `0:0` | `flow_mbpo_awac_replay_family_seed0_l40s_20260602.csv`, L40S / embers, 4 CPUs | `9b5319d24f92ad0a4ebd40814905f771affc8628` | Same replay-family AWAC config | Same env/dataset/replays | 0 | L40S / `embers` | W&B disabled | Per-row output dirs under `flow_mbpo_awac_replay_family_seed0_l40s_20260602/` | `summary.json`; real-eval snapshots; no video | traj_h3 `19.2495 / 289.000 / 1.000`; traj_h5 `19.3567 / 290.500 / 1.000`; residual_h3 `19.4806 / 292.000 / 1.000`; residual_h5 `19.4697 / 291.500 / 1.000`; mlp_ref_h1 `19.3806 / 290.625 / 1.000`; all BC gate false | None; configured early stop at iter 20 | Usable fixed-SHA negative diagnostic | No promotion |

Interpretation: H100/L40S fixed-SHA endpoint and replay-family results match
the H200 conclusion. AWAC is mechanically stable on all tested GPU classes, but
the current setting is uniformly negative against the BC gate and should not
receive formal eval/video budget.

## Continuation Inventory: Full-Upstream PWM MJLab Formal Eval/Rollout Submissions

Submission time: 2026-06-03 after commit `6d6949a`, America/New_York.

Purpose: run the complete upstream `baselines/PWM/scripts/train_dflex.py` /
`pwm.algorithms.pwm.PWM.train()` MJLab bridge checkpoint through formal
final/best real-environment eval and rollout-video gates. This is distinct from
the older `run_original_pwm_adapter.py` proxy/adapted PWM path.

| Job ID | Purpose | Status | Command / script | Git SHA | Config | Env / dataset / version | Seed | GPU / QOS | W&B link or offline dir | Checkpoint paths | Eval / video paths | Return / length / fall | Failure reason | Usable? | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `9402743_[0-1%2]` | Full-upstream PWM final/best eval40 | `PENDING` at first poll, reason `Resources` | `submit_upstream_pwm_mjlab_formal_eval_rollout_20260603.sh`, `MODE=eval`, H200 / `embers` | `6d6949a` exported at submit time | Existing full upstream longdiag checkpoint from `9401906`; eval episodes 40, 16 envs, max steps 1000 | MJLab G1 env config saved under `baselines/PWM/scripts/outputs/2026-06-02/21-35-04/.hydra/config.yaml` | 0 | H200 / `embers` | W&B disabled | `.../logs/upstream_pwm_mjlab_full_longdiag_h200_seed0_20260602/{final_policy.pt,best_policy.pt}` | `scripts/outputs/mjlab_qs/upstream_pwm_full_pipeline_formal_eval40_h200_20260603/{final,best}/summary.json` pending | Pending | None yet | Pending formal diagnostic | Monitor |
| `9402742_[0-1%2]` | Full-upstream PWM final/best rollout10 videos | `PENDING` at first poll, reason `Resources` | Same submit script, `MODE=rollout`, H200 / `embers` | `6d6949a` exported at submit time | Existing full upstream longdiag checkpoint from `9401906`; rollout episodes 10, max steps 1000, MP4 required | Same saved MJLab/Hydra config | 0 | H200 / `embers` | W&B disabled | Same final/best checkpoints | `scripts/outputs/mjlab_qs/upstream_pwm_full_pipeline_rollout10_h200_20260603/{final,best}/summary.json` and `rollout.mp4` pending | Pending | None yet | Pending formal video diagnostic | Monitor |
| `9402746_[0-1%2]` | H100 backup full-upstream eval40 | `PENDING` at first poll, reason `None` | Same submit script, `MODE=eval`, H100 / `embers` | `6d6949a` exported at submit time | Same eval40 protocol | Same saved MJLab/Hydra config | 0 | H100 / `embers` | W&B disabled | Same final/best checkpoints | `scripts/outputs/mjlab_qs/upstream_pwm_full_pipeline_formal_eval40_h100_20260603/{final,best}/summary.json` pending | Pending | None yet | Pending backup diagnostic | Monitor |
| `9402747_[0-1%2]` | H100 backup full-upstream rollout10 | `PENDING` at first poll, reason `None` | Same submit script, `MODE=rollout`, H100 / `embers` | `6d6949a` exported at submit time | Same rollout10 protocol | Same saved MJLab/Hydra config | 0 | H100 / `embers` | W&B disabled | Same final/best checkpoints | `scripts/outputs/mjlab_qs/upstream_pwm_full_pipeline_rollout10_h100_20260603/{final,best}/summary.json` and `rollout.mp4` pending | Pending | None yet | Pending backup diagnostic | Monitor |
| `9402744_[0-1%2]` | L40S backup full-upstream eval40 | `PENDING` at first poll, reason `None` | Same submit script, `MODE=eval`, L40S / `embers` | `6d6949a` exported at submit time | Same eval40 protocol | Same saved MJLab/Hydra config | 0 | L40S / `embers` | W&B disabled | Same final/best checkpoints | `scripts/outputs/mjlab_qs/upstream_pwm_full_pipeline_formal_eval40_l40s_20260603/{final,best}/summary.json` pending | Pending | None yet | Pending backup diagnostic | Monitor |
| `9402745_[0-1%2]` | L40S backup full-upstream rollout10 | `PENDING` at first poll, reason `None` | Same submit script, `MODE=rollout`, L40S / `embers` | `6d6949a` exported at submit time | Same rollout10 protocol | Same saved MJLab/Hydra config | 0 | L40S / `embers` | W&B disabled | Same final/best checkpoints | `scripts/outputs/mjlab_qs/upstream_pwm_full_pipeline_rollout10_l40s_20260603/{final,best}/summary.json` and `rollout.mp4` pending | Pending | None yet | Pending backup diagnostic | Monitor |

Cancellation/replacement update: the W&B-disabled arrays above were canceled
before start because formal gates require W&B-backed eval records and MP4/W&B
videos. `sacct` recorded all six as `CANCELLED by 3509929`, elapsed `00:00:00`.
Commit `0d64c84` added W&B logging to the full-upstream eval/render runners and
changed the submit wrapper to use W&B-on output roots.

Replacement submission time: 2026-06-03 after commit `0d64c84`,
America/New_York. The `sbatch` client reported socket timeouts, but `squeue` and
`sacct` confirmed that all replacement arrays were accepted.

| Job ID | Purpose | Status | Command / script | Git SHA | Config | Env / dataset / version | Seed | GPU / QOS | W&B link or offline dir | Checkpoint paths | Eval / video paths | Return / length / fall | Failure reason | Usable? | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `9402769_[0-1%2]` | W&B-on full-upstream PWM final/best eval40 | `PENDING` at first poll, reason `Resources` | `submit_upstream_pwm_mjlab_formal_eval_rollout_20260603.sh`, `MODE=eval`, H200 / `embers` | `0d64c84` exported at submit time | Existing full upstream longdiag checkpoint from `9401906`; eval episodes 40, 16 envs, max steps 1000 | Saved MJLab/Hydra config under `baselines/PWM/scripts/outputs/2026-06-02/21-35-04/.hydra/config.yaml` | 0 | H200 / `embers` | W&B project `flow-mbpo-mjlab-full-upstream-pwm`, group `upstream_pwm_mjlab_full_pipeline_20260603_eval` | `.../logs/upstream_pwm_mjlab_full_longdiag_h200_seed0_20260602/{final_policy.pt,best_policy.pt}` | `scripts/outputs/mjlab_qs/upstream_pwm_full_pipeline_formal_eval40_wandb_h200_20260603/{final,best}/summary.json` pending | Pending | None yet | Pending formal gate | Monitor |
| `9402771_[0-1%2]` | W&B-on full-upstream PWM final/best rollout10 videos | `PENDING` at first poll, reason `Resources` | Same submit script, `MODE=rollout`, H200 / `embers` | `0d64c84` exported at submit time | Same checkpoint; rollout episodes 10, max steps 1000, MP4 required | Same saved MJLab/Hydra config | 0 | H200 / `embers` | W&B project `flow-mbpo-mjlab-full-upstream-pwm`, group `upstream_pwm_mjlab_full_pipeline_20260603_rollout` | Same final/best checkpoints | `scripts/outputs/mjlab_qs/upstream_pwm_full_pipeline_rollout10_wandb_h200_20260603/{final,best}/summary.json` and `rollout.mp4` pending | Pending | None yet | Pending formal video gate | Monitor |
| `9402774_[0-1%2]` | H100 W&B-on backup full-upstream eval40 | `PENDING` at first poll, reason `Priority` | Same submit script, `MODE=eval`, H100 / `embers` | `0d64c84` exported at submit time | Same eval40 protocol | Same saved MJLab/Hydra config | 0 | H100 / `embers` | Same W&B project/eval group | Same final/best checkpoints | `scripts/outputs/mjlab_qs/upstream_pwm_full_pipeline_formal_eval40_wandb_h100_20260603/{final,best}/summary.json` pending | Pending | None yet | Pending backup formal gate | Monitor |
