...
`--critic-random-actions K`, the critic samples `K` OOD actions per state,
evaluates actor plus sampled OOD actions, and uses a temperature-scaled logsumexp
OOD value against `Q(data)`. The default OOD source is uniform actions in
`[-1, 1]`, preserving the original behavior. The opt-in
`--critic-ood-action-source {uniform,data_noise,mixed}` and
`--critic-action-noise-std` settings support local data-noise OOD actions and a
uniform/local mixture for sampled-action coverage diagnostics. Defaults keep the
earlier actor-only CQL path when `--critic-random-actions 0`. A CPU fake-data
smoke with `critic_random_actions=4` passed, and W&B-disabled job `9356862` ran
`20` iterations on the support-aware generated H3 replay with `K=10`,
conservative-Q weight `1.0`, CQL temperature `1.0`, and actor critic weight
`0.01`. Final metrics were critic loss `2.41859`, Bellman loss
